...
export default function PermainanEdit() {
  ...
  useEffect(() => {
    const getPermainanID = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/Permainan/${id}`);
  ...
  const updatePermainan = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:5000/Permainan/${id}`, {
        nama,
        namalain,
        provinsi,
        kota,
  ...
  return (
    <div className="font-romans text-secondary py-3 px-2">
      ...
      <div>
        <form onSubmit={updatePermainan} className="w-full mb-10">
