...
export default function TradisiEdit() {
  ...
  useEffect(() => {
    const getTradisiID = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/Tradisi/${id}`);
  ...
  const updateTradisi = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(
        `http://localhost:5000/Tradisi/${id}`,
        {
          nama,
          namalain,
          provinsi,
  ...
  return (
    <div className="font-romans text-secondary py-3 px-2">
      ...
      <div>
        <form onSubmit={updateTradisi} className="w-full mb-10">
