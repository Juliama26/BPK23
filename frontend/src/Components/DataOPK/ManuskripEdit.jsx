...
export default function ManuskripEdit() {
  ...
  useEffect(() => {
    const getManuskripID = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/Manuskrip/${id}`
        );
  ...
  return (
    <div className="font-romans text-secondary py-3 px-2">
      ...
      <div>
        <form onSubmit={updateManuskrip} className="w-full mb-10">
