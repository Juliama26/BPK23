...
export default function PengetahuanEdit() {
  ...
  useEffect(() => {
    const getPengetahuanID = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/Pengetahuan/${id}`);
  ...
  const updatePengetahuan = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:5000/Pengetahuan/${id}`, {
        nama,
        namalain,
        provinsi,
        kota,
  ...
  return (
    <div className="font-romans text-secondary py-3 px-2">
      ...
      <div>
        <form onSubmit={updatePengetahuan} className="w-full mb-10">
