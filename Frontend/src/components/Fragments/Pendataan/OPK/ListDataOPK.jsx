...
export default function ListDataOPK() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BE_URL}/data-opk/category/${id}`,
        {
          headers: {
  ...
  const exportToExcel = () => {
    const dataToExport = filteredList.map((item, index) => ({
      ...
      NilaiMoral: item.nilai_moral,
      Aturan: item.aturan,
      Jml_pemain: item.jml_pemain,
      ...
      Dokumen_Lainnya: item.documenUrl,
    }));
    ...
    XLSX.utils.book_append_sheet(wb, ws, `${list[0]?.opkCategory?.title}`);
    XLSX.writeFile(wb, `${list[0]?.opkCategory?.title}.xlsx`);
  };
  ...
  return (
    <section className="min-h-screen">
      ...
      <h2 className="text-sm py-2 font-semibold opacity-50 uppercase">
        <span className="text-400">OPK / </span>
        {list[0]?.opkCategory?.title || "?"}
      </h2>
